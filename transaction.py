...
def profile_labeled_data(file_path:str, save_path:str):
    ...
    for file_name in file_names:
        ...
        # value
        send_value_mean = df.loc[df["from_title"] == "self", 'value'].values.astype(float).mean()
        ...
        # address, transaction count, transaction per day, from category, to category, from title, to title, transaction in/out, Average in value, Average out value Average gas, First transaction, Interact DApp Sequence
        store_list.append([file_name[:-12], transaction_count, transaction_per_day, from_category_dict, to_category_dict, from_title_dict, to_title_dict, send_count, receive_count, send_value_mean, receive_value_mean, average_gas, first_date, app_sequence])
    ...
    store_df = pd.DataFrame(store_array, index = None, columns=["address", "transaction_count", "transaction_per_day", "from_cate", "to_cate", "from_title", "to_title", "send_count", "receive_count", "send_value_mean", "receive_value_mean", "average_gas", "first_date", "app_sequence"]).fillna(0)
...
if __name__ == '__main__':
    ...
    # profile_labeled_data("./transaction/CryptokittySiringAuction_labeled/", "./transaction/profiled/CryptokittySiringAuction4000.csv")
